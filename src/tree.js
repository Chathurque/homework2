...
configFun.prototype = {
    ...
    tree:function(res){
    	...
    	for(var i=0; i<data.length; i++){
    		if(data[i].pid == 0){
				var fatherHtml = $('<div class="TreeListWrapper"><div class="TreeList-line"></div></div>');
				var listHtml = '';
				if(res.type == 0){
					listHtml = $('<div class="TreeList TreeListOne">'+
							'<div class="TreeList-top">'+
							'<div class="TreeList-old"><span class="f-iconJia"></span></div>'+
							'<div class="TreeList-title">'+
							'<div class="TreeList-titleImg"><span class="f-iconBi"></span></div>'+
							'<p class="TreeList-titleP">'+ data[i].title +'</p>'+
							'</div></div></div>');
					};
				if(res.type == 1){
    	...
    	for(var i=0; i<one.length; i++){
    		if(i == one.length-1){
    			one[i].list.siblings('.TreeList-line').eq(0).remove();
    		};
    		(function(k){
    			var selfList = one[k].list;
    			var old = selfList.find('.TreeList-old').eq(0);
    			old[0].onclick = function(){
	    			var iconDiv = old.children('span').eq(0);
	    			var flagClass = iconDiv.attr('class');
	    			var listWrapper = old.parents('.TreeList-top').eq(0).siblings('.TreeListWrapper').eq(0);
	    			old.addClass('f-on');
    ...
    treeFor:function(res){
    	...
    	var fatherHtml = $('<div class="TreeListWrapper hide"><div class="TreeList-lineShu"></div></div>');
    	var listHtml = '';
    	var self = [];//子集
    	var next = [];
    	for(var i=0; i<data.length; i++){
    	...
    	if(self.length == 0 && res.dataHover.pid != 0){
    		...
    		var selfWrapper = selfDom.parent('.TreeListWrapper');//底级所在的容器
    		var selfList = selfWrapper.parent('.TreeList');
    		var fatherOld = selfList.children('.TreeList-top').eq(0).children('.TreeList-old').eq(0);
    		var selfNextDom = selfDom.next();
    		var line = $('<div class="TreeList-lineEnd"></div>');
    		var selfDom_top = selfDom.find('.TreeList-top').eq(0);
    		var selfDom_old = selfDom.find('.TreeList-old').eq(0);
    		var selfDom_img = selfDom.find('.TreeList-titleImg').eq(0).children('span').eq(0);
    		if(typeof(selfNextDom[0]) == 'undefined'){
    			var fatherLine = selfDom.siblings('.TreeList-lineShu');
    			fatherLine.removeClass(fatherLine.attr('class'));
    			fatherLine.addClass('TreeList-lineShuEnd');
    		};
    		...
    		//绑定radio点击事件
    		if(res.type == 1){
    			var radio = selfDom.find('.TreeList-radio').eq(0);
    			var radio_icon = radio.children('span').eq(0);
    			var radioFather = selfList.children('.TreeList-top').eq(0).find('.TreeList-radio').eq(0);
    			var radioFather_icon = radioFather.children('span').eq(0);
    			radio[0].onclick = function(){
    				var radio_father = radio.parents('.TreeListWrapper');
    				...
    				if(radio_class == 'f-iconRadio'){
    					...
    					for(var i=0; i<radio_father.length-1; i++){
	    					var radio_father_radio = radio_father.eq(i).siblings('.TreeList-top').eq(0).find('.TreeList-radio').eq(0);
    					...
    					for(var i=0; i<radio_father.length-1; i++){
	    					var radio_father_radio = radio_father.eq(i).siblings('.TreeList-top').eq(0).find('.TreeList-radio').eq(0);
	    					var radio_father_radioIcon = radio_father_radio.children('span').eq(0);
	    					var radio_fatherList = radio_father.eq(i).children('.TreeList');
	    					var flagNum = 0;
	    					//判断子级是否被选中
	    					for(var k=0; k<radio_fatherList.length; k++){
	    						var radio_fatherList_radio = radio_fatherList.eq(k).children('.TreeList-top').eq(0).find('.TreeList-radio').eq(0);
    			...
    			radioFather[0].onclick = function(){
    				...
    				var radio_next = selfWrapper.find('.TreeList-radio');
    ...
    treeJson:function(res){
    	var wrapper = $(res.wrapper);
    	var radio = wrapper.find('.TreeList-radio');
    	var radioArr = [];
